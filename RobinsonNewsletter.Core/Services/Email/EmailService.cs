...
{
    public async Task SendMailFromTemplateAsync<T>(T model, CancellationToken cancellationToken = default)
    {
        ...
        var msg = MailHelper.CreateSingleTemplateEmailToMultipleRecipients(new EmailAddress(emailSettings.SenderEmail, emailSettings.SenderEmail),
                                                                           emailSettings.Recipients.Select(e => new EmailAddress(e)).ToList(),
